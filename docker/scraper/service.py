...
class AuditService:
    ...
    def classify_and_scrape(self, url: str) -> tuple[any, str]:
        """
        ...
        for nodo_id in spa_nodos:
            ...
            if el:
                ...
                if text_len < 100:
                    logger.info("🎯 Detectado contenedor SPA '%s' con poco contenido (%d chars).", el.name or el.get('id'), text_len)
        ...
        if word_count < 80 and script_char_count > 5000:
            logger.info("🎯 Detectado contenido muy ligero (%d palabras) con JS pesado (%d chars).", word_count, script_char_count)
    ...
    def process_website(self, entry: dict) -> bool:
        ...
        if strategy_key == "auto":
            ...
            if result and result.status == "success":
                ...
                if (result is None or result.status != "success") and self._last_bs_prefetch and self._last_bs_prefetch.status == "success":
                    logger.info("🚀 [FAILSAFE] Selenium o ejecución principal falló. Recuperando pre-fetch exitoso de BeautifulSoup.")
