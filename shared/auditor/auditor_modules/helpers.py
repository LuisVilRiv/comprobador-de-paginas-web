...
def attr_to_str(value: str | list[str] | None) -> str:
    """Safely convert a BeautifulSoup tag attribute to a plain string.

    BS4's Tag.get() can return str, list[str], or None depending on the
    attribute.  This helper normalises the value so callers can safely
    call .strip(), .lower(), etc. without type errors.
    """
    if value is None:
        return ""
