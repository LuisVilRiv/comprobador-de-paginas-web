...
def status_from_score(score: int) -> str:
    if score >= settings.AUDIT_SCORE_EXCELLENT_THRESHOLD:
        return "excelente"
    if score >= settings.AUDIT_SCORE_GOOD_THRESHOLD:
        return "bueno"
    if score >= 50:
        return "mejorable"
    return "crítico"
...
def evaluate_release_gate(
    ...
    if score < settings.AUDIT_RELEASE_GATE_MIN_SCORE:
        blockers.append(
            f"Puntuación global insuficiente para producción ({score}/{settings.AUDIT_RELEASE_GATE_MIN_SCORE})."
        )
...
def build_recommendations(
    ...
    def has_issues(lst):
        return any("sin incidencias" not in i for i in lst)

    if has_issues(security_issues):
        recs.append("Reforzar la seguridad HTTP: cabeceras, HTTPS, SRI.")
    if has_issues(seo_issues):
        recs.append("Corregir metadatos SEO: title, description, canonical.")
    if has_issues(structure_issues):
        recs.append("Reforzar la estructura semántica y jerarquía de encabezados.")
    if has_issues(image_issues):
        recs.append("Arreglar imágenes rotas y completar atributos alt.")
    if has_issues(content_issues):
        recs.append("Eliminar contenido de relleno, incoherente o inadecuado.")
    if has_issues(link_issues):
        recs.append("Corregir los enlaces rotos.")
    if has_issues(button_issues):
        recs.append("Revisar botones y formularios.")
    if has_issues(technical_issues):
        recs.append("Aplicar hardening técnico: doctype, charset, mixed content.")
    return recs
