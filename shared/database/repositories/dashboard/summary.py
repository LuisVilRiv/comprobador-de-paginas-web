...
from shared.database.models import AuditIssue, AuditRun, Client, Website
...
def global_summary() -> dict:
    with get_db() as db:
        ...
        pending_audit_count = (
            db.execute(select(func.count(Website.id)).where(Website.pending_audit == True)).scalar() or 0
        )
