...
def list_websites(client_id: str | None = None) -> list[dict]:
    with get_db() as db:
        ...
        latest_ids = (
            select(AuditRun.website_id.label("website_id"), func.max(AuditRun.started_at).label("max_started"))
        ...
        stmt = (
            select(
                ...
                AuditRun.status.label("run_status"),
