    ...
    try:
        with get_db() as db:
            ...
            if run:
                ...
                db.commit()
                logger.debug("Run %s updated to %d/%d", run_id, passed, total)
            else:
                logger.debug("Run %s not found for progress update", run_id)
    except Exception as e:
        logger.error("Error updating progress for run %s: %s", run_id, e)
...
def save_audit_run(
    ...
    with get_db() as db:
        ...
        try:
            ...
            logger.warning("Error en cleanup de auditorías antiguas: %s", exc)
