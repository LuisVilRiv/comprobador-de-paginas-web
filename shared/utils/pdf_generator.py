...
def _build_single_section_chart(k: str, label: str, series: list[dict], i: int) -> io.BytesIO:
    ...
    ax.plot(x_coords, vals, marker="o", markersize=6, linewidth=2.5, color=color, zorder=3)
...
def generate_audit_pdf(run, website, history: list[dict] | None = None) -> io.BytesIO:
    ...
    for title, desc in [
        (
            "🛡️ Seguridad:",
            "Verificación de cabeceras HTTP, exposición de versiones del servidor y configuraciones críticas para prevenir ataques.",
        ),
        (
            "🔍 SEO:",
            "Evaluación de metadatos (Title, Description) y etiquetas de rastreo para asegurar la visibilidad en motores de búsqueda.",
        ),
        (
            "⚡ Rendimiento:",
            "Análisis del tiempo de respuesta inicial y estabilidad del servidor para garantizar una experiencia de usuario fluida.",
        ),
        (
            "🏗️ Estructura HTML:",
            "Revisión de la jerarquía semántica (H1-H6) y validación de estándares para una correcta lectura de navegadores.",
        ),
        (
            "♿ Accesibilidad y Contenido:",
            "Comprobación de textos alternativos (alt) en imágenes, densidad de texto y calidad del contenido visual.",
        ),
        (
            "🔗 Enlaces y Navegación:",
            "Detección de enlaces rotos (404) y verificación de la integridad de los elementos interactivos del sitio.",
        ),
    ]:
...
def generate_client_report(client_name: str, websites_data: list[dict]) -> io.BytesIO:
    ...
    elements.append(Paragraph("Reporte Consolidado de Auditoría Web", styles["CenterTitle"]))
    ...
    for w_data in websites_data:
        ...
        if run.issues:
            ...
            if critical_issues:
                ...
                elements.append(
                    Paragraph("No se encontraron hallazgos críticos ni altos en la última ejecución.", styles["Normal"])
                )
