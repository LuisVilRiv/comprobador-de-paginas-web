...
def test_get_settings(mock_get_settings, client):
    ...
    mock_get_settings.return_value = {"cron_active": "0 0 * * *", "cron_inactive": "0 12 * * *"}

    response = client.get("/settings")
    assert response.status_code == 200
    assert response.json() == {"cron_active": "0 0 * * *", "cron_inactive": "0 12 * * *"}
...
def test_update_settings(mock_update_settings, client):
    ...
    response = client.put("/settings", json={"cron_active": "*/5 * * * *", "cron_inactive": "0 0 * * 0"})

    assert response.status_code == 200
    assert response.json() == {"message": "Configuración actualizada correctamente"}
    mock_update_settings.assert_called_once_with(cron_active="*/5 * * * *", cron_inactive="0 0 * * 0")
...
def test_get_clients(mock_list_clients, client):
    ...
    mock_list_clients.return_value = [{"id": "1", "name": "Luis Vilches", "email": "luis@mail.com"}]

    response = client.get("/clients")
    assert response.status_code == 200
    assert response.json() == [{"id": "1", "name": "Luis Vilches", "email": "luis@mail.com"}]
...
def test_create_client(mock_create_client, client):
    ...
    mock_create_client.assert_called_once_with(
        "Nuevo Cliente", "nuevo@mail.com", "123456", "Mi Compañía", "Notas", None
    )
...
def test_get_websites(mock_list_websites, client):
    ...
    mock_list_websites.return_value = [{"website_id": "10", "url": "https://test.com", "strategy": "auto"}]

    response = client.get("/websites")
    assert response.status_code == 200
    assert response.json() == [{"website_id": "10", "url": "https://test.com", "strategy": "auto"}]
...
def test_create_website(mock_create_website, client):
    ...
    mock_create_website.assert_called_once_with(
        "1", "https://new-web.com", "Nueva Web", "beautifulsoup", True, "0 0 * * *"
    )
